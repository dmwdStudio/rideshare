  ...
  $vUserDeviceCountry = isset($_REQUEST["vUserDeviceCountry"]) ? $_REQUEST["vUserDeviceCountry"] : '';
	...
	function getPassengerDetailInfo($passengerID,$cityName){
      ...
      $row[0]['SERVER_MAINTENANCE_ENABLE'] = $row[0]['MAINTENANCE_APPS'];
			...
			$row[0]['APP_PAYMENT_MODE'] = $generalobj->getConfigurations('configurations','RIDER_APP_PAYMENT_MODE');
	...
	function getDriverDetailInfo($driverId,$fromSignIN=0){
      ...
      $Data[0]['SERVER_MAINTENANCE_ENABLE'] = $Data[0]['MAINTENANCE_APPS'];
        ...
        $returnArr['eStatus'] =$Data[0]['eStatus'];
			...
			$Data[0]['APP_PAYMENT_MODE'] = $generalobj->getConfigurations('configurations','DRIVER_APP_PAYMENT_MODE');
    ...
    if($vLang != ""){
      ...
      $DataArr['DefaultLanguageValues']=$requireLangValues[0];
		...
		if($UserType == "Passenger"){
			...
			$DataArr['APP_PAYMENT_MODE'] = $generalobj->getConfigurations("configurations", "RIDER_APP_PAYMENT_MODE");
			}else{
			...
			$DataArr['APP_PAYMENT_MODE'] = $generalobj->getConfigurations("configurations", "DRIVER_APP_PAYMENT_MODE");
		} 
			...
			if($vTripPaymentMode == "Card" && $fCancellationFare > 0){
				...
				try{
					if($fCancellationFare > 0){
						$charge_create = \Stripe\Charge::create(array(
						"amount" => $price_new,
						"currency" => $currency,
						...
						if($fCancellationFare == 0 || ($charge_create->status=="succeeded" && $charge_create->paid=="1")){
	...
	if ($type == "updateDriverStatus") {
		...
		$vTimeZone = isset($_REQUEST["vTimeZone"]) ? $_REQUEST["vTimeZone"] : '';
		//$APP_TYPE = $generalobj->getConfigurations("configurations", "APP_TYPE");
		$APP_PAYMENT_MODE = $generalobj->getConfigurations("configurations", "DRIVER_APP_PAYMENT_MODE");
	...
	if($type=="CollectPayment"){
		...
		if($vTripPaymentMode == "Card" && $isCollectCash == ""){
			...
			try{
				...
				if($iFare == 0 || ($charge_create->status=="succeeded" && $charge_create->paid=="1")){
    ...
    $currencyratio = $DefaultCurrencyData[0]['Ratio'];
		...
		try{
			...
			$details = json_decode($charge_create);
			$result = get_object_vars($details);
      if($charge_create->status=="succeeded" && $charge_create->paid=="1"){
    ...
    $vStripeCusId = $UserDetail[0]['vStripeCusId']; 
		...
		try{
			if($vStripeCusId  != ""){
				$customer 	= \Stripe\Customer::retrieve($vStripeCusId);
				...
				try{     
        	$customer 	= \Stripe\Customer::create(array( "source" => $vStripeToken, "email" => $vEmail));
